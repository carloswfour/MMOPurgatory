...
-- Modified:
-- 12/Jul/2015 - Added expit and rehandle tonnes and load counts
-- =============================================
CREATE FUNCTION [dbo].[equip_kpi_hour_per_unit]
(	
	@shift_date datetime,
...
RETURN (
	...
	LEFT JOIN (
	SELECT dump_end_shift_date AS shift_date, dump_end_shift_ident as shift_ident, equip_ident, eqmodel_code, SUM(quantity_reporting) AS wet_tonnes, COUNT(*) AS num_loads, SUM(expit_tonnes) AS expit_tonnes, SUM(expit_loads) AS expit_loads, SUM(rehandle_tonnes) AS rehandle_tonnes, SUM(rehandle_loads) AS rehandle_loads
	FROM (
		SELECT dump_end_shift_date, dump_end_shift_ident, dump_end_timestamp, loading_unit_ident, hauling_unit_ident, quantity_reporting,
		CASE WHEN LOAD_LOCATION_SNAME LIKE '%.%-%' THEN quantity_reporting ELSE 0 END AS expit_tonnes,
		CASE WHEN LOAD_LOCATION_SNAME LIKE '%.%-%' THEN 1 ELSE 0 END AS expit_loads,
		CASE WHEN LOAD_LOCATION_SNAME NOT LIKE '%.%-%' THEN quantity_reporting ELSE 0 END AS rehandle_tonnes,
		CASE WHEN LOAD_LOCATION_SNAME NOT LIKE '%.%-%' THEN 1 ELSE 0 END AS rehandle_loads
	...
	GROUP BY dump_end_shift_date, dump_end_shift_ident, e.equip_ident, eqmodel_code
	) AS movements
