...
create VIEW [dbo].[Z_EQUIP_KPI_BY_EQUIP]
AS
...
--
-- KDaws 11Nov12 - Created to [Z_EQUIP_KPI_BY_EQUIP]
-- =============================================
...
FROM (
	...
	INNER JOIN (
		SELECT eqmodel_code, descrip, equip_ident, COUNT(eqmodel_code) AS fleet_count
		FROM WencoReport.dbo.equip
		WHERE active = 'Y' AND test = 'N'
		GROUP BY eqmodel_code, descrip, equip_ident
	) AS e 
	ON e.equip_ident = ZESS.equip_ident AND fleet_count > 0
	GROUP BY
	ZESS.shift_date,
	ZESS.shift_ident,
	ZESS.equip_ident,
...
GROUP BY dump_end_shift_date, dump_end_shift_ident, eqmodel_code, descrip, e.equip_ident
) AS movements
